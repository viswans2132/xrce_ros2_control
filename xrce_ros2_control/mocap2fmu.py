...
class MocapOdom(Node):
	def __init__(self):
		...
		self.px4_odom_sub = self.create_subscription(VehicleOdometry, '/fmu/out/vehicle_odometry', self.find_odometry, self.qos_profile_1)
		...
		if no_rigid_bodies > 0:
			for rigidbody in msg.rigidbodies:
				if rigidbody.rigid_body_name == 'ShafterX2.ShafterX2':
					...
					self.vicon_pos = np.array([rigidbody.pose.position.x, -rigidbody.pose.position.y, -rigidbody.pose.position.z])


					self.odom_pub.publish(odom_msg)
					# print(f'Vicon Position: {odom_msg.position}')
	...
	def find_odometry(self, msg):
		errPos = self.vicon_pos - np.array([msg.position[0], msg.position[1], msg.position[2]])
		print(errPos)
